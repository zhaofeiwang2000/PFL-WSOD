...
from roi_data.loader_inter import RoiDataLoader, MinibatchSampler, BatchSampler, collate_minibatch
from modeling.model_builder_inter import Generalized_RCNN
    ...
    try:
        ...
        for step in range(args.start_step, cfg.SOLVER.MAX_ITER):
            ...
            for inner_iter in range(args.iter_size):
                ...
                # import pdb; pdb.set_trace()
                pcl.set_inner_iter(step)
                img    = input_data['data'][0].cuda()
                rois   = input_data['rois'][0].cuda()
                labels = input_data['labels'][0].cuda()
                id = input_data['id']
                net_outputs = pcl(img, rois, labels, id)
                # import pdb; pdb.set_trace()
